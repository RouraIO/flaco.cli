...
# flacoai benchmark harness

flacoai uses benchmarks to quantitatively measure how well it works
with various LLMs.
...
This
benchmark evaluates how effectively flacoai and LLMs can translate a
...
and *format those code edits* so that flacoai can save the
edits to the local source files.

See [this writeup for a longer discussion about the benchmark](https://flacoai.chat/2024/12/21/polyglot.html).
...
There are 3 main tasks involved in benchmarking flacoai:
...
```
# Clone the flacoai repo
git clone https://github.com/flacoai-AI/flacoai.git

# Create the scratch dir to hold benchmarking results inside the main flacoai dir:
cd flacoai
mkdir tmp.benchmarks

# Clone the repo with the exercises
git clone https://github.com/flacoai-AI/polyglot-benchmark tmp.benchmarks/polyglot-benchmark
...
# Inside the container, install flacoai as a development build.
...
- `--model` is the name of the model, same as you would pass directly to `flacoai`.
- `--edit-format` is the name of the edit format, same as you would pass directly to `flacoai`. When working with an experimental LLM, I recommend starting with `whole`
...
- `--read-model-settings=<filename.yml>` specify model settings, see here: https://flacoai.chat/docs/config/adv-model-settings.html#model-settings
...
[flacoai leaderboard data files](https://github.com/flacoai-AI/flacoai/blob/main/flacoai/website/_data/).
...
[flacoai leaderboard data files](https://github.com/flacoai-AI/flacoai/blob/main/flacoai/website/_data/).
- These scripts are not intended for use by typical flacoai end users.
