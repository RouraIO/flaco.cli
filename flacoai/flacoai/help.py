...
from flacoai.help_pats import exclude_website_pats
...
def get_package_files():
    for path in importlib_resources.files("flacoai.website").iterdir():
        if path.is_file():
            yield path
        elif path.is_dir():
...
def get_index():
    ...
    if index is None:
        ...
        for fname in get_package_files():
            ...
            doc = Document(
                text=importlib_resources.files("flacoai.website")
                .joinpath(fname)
