...
from flacoai import prompts, utils
...
class GitRepo:
    repo = None
    flacoai_ignore_file = None
    flacoai_ignore_spec = None
    flacoai_ignore_ts = 0
    flacoai_ignore_last_check = 0
    subtree_only = False
    ignore_file_cache = {}
    git_repo_error = None
    ...
    def __init__(
        ...
        if flacoai_ignore_file:
            self.flacoai_ignore_file = Path(flacoai_ignore_file)

    def commit(self, fnames=None, context=None, message=None, flacoai_edits=False, coder=None):
        """
        ...
        Args:
            ...
            flacoai_edits (bool, optional): Whether the changes were made by flacoai. Defaults to False.
        ...
        flacoai generated the changes (`flacoai_edits`).
        ...
        - flacoai_edits=True: Changes were generated by flacoai (LLM).
        - flacoai_edits=False: Commit is user-driven (e.g., /commit manually staged changes).
        ...
        Flags:
        - --attribute-author: Modify Author name to "User Name (flacoai)".
        - --attribute-committer: Modify Committer name to "User Name (flacoai)".
        - --attribute-co-authored-by: Add
          "Co-authored-by: flacoai (<model>) <flacoai@flacoai.chat>" trailer to commit message.

        Behavior Summary:

        1. When flacoai_edits = True (AI Changes):
        ...
        2. When flacoai_edits = False (User Changes):
           - --attribute-co-authored-by is IGNORED (trailer never added).
           - Author name is NEVER modified (--attribute-author ignored).
           - Committer name IS modified by default (implicit True, as flacoai runs `git commit`).
           - EXCEPTION: If --attribute-committer is EXPLICITLY False, the name is NOT modified.

        Resulting Scenarios:
        - Standard AI edit (defaults): Co-authored-by=False -> Author=You(flacoai),
          Committer=You(flacoai)
        ...
        - AI edit with Co-authored-by + Explicit Author: Co-authored-by=True,
          --attribute-author -> Author=You(flacoai), Committer=You, Trailer added
        - User commit (defaults): flacoai_edits=False -> Author=You, Committer=You(flacoai)
        - User commit with explicit no-committer: flacoai_edits=False,
        ...
        prefix_commit_message = flacoai_edits and (
        ...
        if flacoai_edits and attribute_co_authored_by:
            model_name = "unknown-model"
            if coder and hasattr(coder, "main_model") and coder.main_model.name:
                model_name = coder.main_model.name
            commit_message_trailer = f"\n\nCo-authored-by: flacoai ({model_name}) <flacoai@flacoai.chat>"

        # Determine if author/committer names should be modified
        # Author modification applies only to flacoai edits.
        # It's used if effective_author is True AND
        # (co-authored-by is False OR author was explicitly set).
        use_attribute_author = (
            flacoai_edits and effective_author and (not attribute_co_authored_by or author_explicit)
        )

        # Committer modification applies regardless of flacoai_edits (based on tests).
        # It's used if effective_committer is True AND
        # (it's not an flacoai edit with co-authored-by OR committer was explicitly set).
        use_attribute_committer = effective_committer and (
            not (flacoai_edits and attribute_co_authored_by) or committer_explicit
        )
        ...
        committer_name = f"{original_user_name} (flacoai)"
    ...
    def refresh_flacoai_ignore(self):
        if not self.flacoai_ignore_file:
            return

        current_time = time.time()
        if current_time - self.flacoai_ignore_last_check < 1:
            return

        self.flacoai_ignore_last_check = current_time

        if not self.flacoai_ignore_file.is_file():
            return

        mtime = self.flacoai_ignore_file.stat().st_mtime
        if mtime != self.flacoai_ignore_ts:
            self.flacoai_ignore_ts = mtime
            self.ignore_file_cache = {}
            lines = self.flacoai_ignore_file.read_text().splitlines()
            self.flacoai_ignore_spec = pathspec.PathSpec.from_lines(
    ...
    def ignored_file_raw(self, fname):
        ...
        if not self.flacoai_ignore_file or not self.flacoai_ignore_file.is_file():
            return False
        ...
        return self.flacoai_ignore_spec.match_file(fname)
