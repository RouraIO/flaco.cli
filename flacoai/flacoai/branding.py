...
def get_flaco_ascii_art(version="1.0.0"):
    ...
    """
    # NOTE: Keep the banner strictly single-column characters inside the box.
    # Many terminals render emoji as double-width, which makes the border look misaligned.
    inner_lines = [
        "",
        "      ███████╗ ██╗      █████╗   ██████╗  ██████╗       █████╗  ██╗",
        "      ██╔════╝ ██║     ██╔══██╗ ██╔════╝ ██╔═══██╗     ██╔══██╗ ██║",
        ...
        "                 Build. Generate. Ship. In your terminal.",
        "",
    ]

    width = max(len(line) for line in inner_lines)

    top = "╔" + ("═" * (width + 2)) + "╗"
    bottom = "╚" + ("═" * (width + 2)) + "╝"
    boxed = [top]
    for line in inner_lines:
        boxed.append("║ " + line.ljust(width) + " ║")
    boxed.append(bottom)
    return "\n".join(boxed)
