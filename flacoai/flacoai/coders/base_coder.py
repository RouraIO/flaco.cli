...
from flacoai import __version__, models, prompts, urls, utils
from flacoai.analytics import Analytics
from flacoai.commands import Commands
from flacoai.exceptions import LiteLLMExceptions
from flacoai.history import ChatSummary
from flacoai.io import ConfirmGroup, InputOutput
from flacoai.linter import Linter
from flacoai.llm import litellm
from flacoai.models import RETRY_TIMEOUT
from flacoai.reasoning_tags import (
    REASONING_TAG,
    format_reasoning_content,
    remove_reasoning_content,
    replace_reasoning_tags,
)
from flacoai.repo import ANY_GIT_ERROR, GitRepo
from flacoai.repomap import RepoMap
from flacoai.run_cmd import run_cmd
from flacoai.utils import format_content, format_messages, format_tokens, is_image_file
...
class Coder:
    ...
    repo = None
    last_flacoai_commit_hash = None
    flacoai_edited_files = None
    ...
    def create(
        ...
        **kwargs,
        ...
        import flacoai.coders as coders
        ...
        if from_coder:
            ...
            update = dict(
                ...
                flacoai_commit_hashes=from_coder.flacoai_commit_hashes,
    ...
    def get_announcements(self):
        ...
        try:
            ...
            if activity and activity.get('session_count', 0) > 0:
                ...
                lines.append(
                    f"   {activity['session_count']} sessions · "
                    f"{activity['files_edited']} files edited · "
                    f"{activity['repos']} repos"
                )
                if activity.get('commands_used'):
                    top_cmds = ', '.join(activity['commands_used'][:3])
                    lines.append(f"   Top commands: {top_cmds}")
        except Exception:
            pass  # Silently skip activity if unavailable
    ...
    def __init__(
        ...
        if flacoai_commit_hashes:
            self.flacoai_commit_hashes = flacoai_commit_hashes
        else:
            self.flacoai_commit_hashes = set()
        ...
        for fname in fnames:
            ...
            if self.repo and self.repo.ignored_file(fname):
                self.io.tool_warning(f"Skipping {fname} that matches flacoaiignore spec.")
                continue
    ...
    def show_announcements(self):
        """Display announcements with optional rich panel formatting."""
        from rich.panel import Panel
        from rich.text import Text

        announcement_lines = self.get_announcements()
    ...
    def warm_cache(self, chunks):
        ...
        delay = 5 * 60 - 5
        delay = float(os.environ.get("flaco_CACHE_KEEPALIVE_DELAY", delay))
    ...
    def send_message(self, inp):
        ...
        if edited:
            self.flacoai_edited_files.update(edited)
    ...
    def auto_commit(self, edited, context=None):
        ...
        try:
            res = self.repo.commit(fnames=edited, context=context, flacoai_edits=True, coder=self)
            if res:
    ...
    def show_auto_commit_outcome(self, res):
        ...
        self.flacoai_commit_hashes.add(commit_hash)
        self.last_flacoai_commit_message = commit_message
        if self.show_diffs:
