...
- flacoai wrote 88% of the code in this release.
...
- Added `--add-gitignore-files` flag to enable adding files listed in .gitignore to flacoai's editing scope, by omarcinkonis.
...
- flacoai wrote 21% of the code in this release.
...
- flacoai wrote 79% of the code in this release.
...
- Prevented flacoai from instructing the LLM to reply in 'C' or 'POSIX' when these are detected as the system locale.
...
- The `flacoai scrape` command-line tool will now use Playwright for web scraping if it is available, by Jon Keys.
...
- The `flacoai-args` utility (or `python -m flacoai.args`) now defaults to printing a sample YAML configuration if no arguments are provided.
...
- flacoai wrote 55% of the code in this release.
...
- flacoai wrote 92% of the code in this release.

### flacoai v0.81.3

- Commit messages generated by flacoai are no longer forced to be entirely lowercase, by Peter Hadlaw.
...
- Fix quoting of values containing '#' in the sample `flacoai.conf.yml`.
- Add support for Fireworks AI model 'deepseek-v3-0324', by Felix Lisczyk.
- Commit messages generated by flacoai are now lowercase, by Anton Ödman.
...
- flacoai wrote 86% of the code in this release.
...
- Handle `FileNotFoundError` if the current working directory is deleted while flacoai is running.
- Fix completion menu current item color styling, by Andrey Ivanov.
- flacoai wrote 87% of the code in this release.
...
- flacoai wrote 93% of the code in this release.

### flacoai v0.79.1

- Improved model listing to include all models in fuzzy matching, including those provided by flacoai (not litellm).
...
- flacoai wrote 65% of the code in this release.
...
- Enhanced `--flacoaiignore` argument to resolve both absolute and relative paths, by mopemope.
...
- Big upgrade in [programming languages supported](https://flacoai.chat/docs/languages.html) by adopting [tree-sitter-language-pack](https://github.com/Goldziher/tree-sitter-language-pack/).
...
- flacoai wrote 72% of the code in this release.
...
- flacoai wrote 75% of the code in this release.
...
- flacoai wrote 0% of the code in this release.
...
- flacoai will notify you when it's completed the last request and needs your input:
  - Added [notifications when LLM responses are ready](https://flacoai.chat/docs/usage/notifications.html) with `--notifications` flag.
...
- flacoai wrote 85% of the code in this release.
...
- flacoai wrote 46% of the code in this release.
...
- Docker containers now set `HOME=/app` which is the normal project mount-point, to persist `~/.flacoai`.
...
- flacoai wrote 77% of the code in this release.

### flacoai v0.73.0

- Full support for o3-mini: `flacoai --model o3-mini`
...
- flacoai wrote 69% of the code in this release.
...
- flacoai wrote 52% of the code in this release.
...
- flacoai wrote 60% of the code in this release.
...
- flacoai wrote 74% of the code in this release.
...
- [Watch files](https://flacoai.chat/docs/usage/watch.html) improvements:
  - Use `# ... AI?` comments to trigger flacoai and ask questions about your code.
  - Now watches *all* files, not just certain source files.
  - Use `# AI comments`, `// AI comments`, or `-- AI comments` to give flacoai instructions in any text file.
- Full support for Gemini Flash 2.0 Exp:
  - `flacoai --model flash` or `flacoai --model gemini/gemini-2.0-flash-exp`
- [New `--multiline` flag and `/multiline-mode` command](https://flacoai.chat/docs/usage/commands.html#entering-multi-line-chat-messages) makes ENTER a soft newline and META-ENTER send the message, by @miradnanali.
- `/copy-context <instructions>` now takes optional "instructions" when [copying code context to the clipboard](https://flacoai.chat/docs/usage/copypaste.html#copy-flacoais-code-context-to-your-clipboard-paste-into-the-web-ui).
...
- flacoai wrote 68% of the code in this release.

### flacoai v0.68.0

- [flacoai works with LLM web chat UIs](https://flacoai.chat/docs/usage/copypaste.html).
  - New `--copy-paste` mode.
  - New `/copy-context` command.
- [Set API keys and other environment variables for all providers from command line or YAML conf file](https://flacoai.chat/docs/config/flacoai_conf.html#storing-llm-keys).
...
- flacoai wrote 71% of the code in this release.

### flacoai v0.67.0

- [Use flacoai in your IDE or editor](https://flacoai.chat/docs/usage/watch.html).
  - Run `flacoai --watch-files` and it will watch for instructions you add to your source files.
  - One-liner `# ...` or `// ...` comments that start or end with "AI" are instructions to flacoai.
  - When flacoai sees "AI!" it reads and follows all the instructions in AI comments.
...
- [flacoai wrote 61% of the code in this release.](https://flacoai.chat/HISTORY.html)
...
- flacoai wrote 82% of the code in this release.
...
- Added `--alias` config to define [custom model aliases](https://flacoai.chat/docs/config/model-aliases.html).
...
- Added [RepoMap support for Dart language](https://flacoai.chat/docs/languages.html) by @malkoG.
- Ask 2.5% of users if they want to opt-in to [analytics](https://flacoai.chat/docs/more/analytics.html).
...
- flacoai wrote 81% of the code in this release.
...
- Added [`/editor` command](https://flacoai.chat/docs/usage/commands.html) to open system editor for writing prompts, by @thehunmonkgroup.
...
- Ask 1% of users if they want to opt-in to [analytics](https://flacoai.chat/docs/more/analytics.html).
- Added support for [optional multiline input tags](https://flacoai.chat/docs/usage/commands.html#entering-multi-line-chat-messages) with matching closing tags.
- Improved [model settings configuration](https://flacoai.chat/docs/config/adv-model-settings.html#global-extra-params) with support for global `extra_params` for `litellm.completion()`.
...
- Full support for Claude 3.5 Haiku
  - Scored 75% on [flacoai's code editing leaderboard](https://flacoai.chat/docs/leaderboards/).
...
- Easily apply file edits from ChatGPT, Claude or other web apps
  ...
  - Run `flacoai --apply-clipboard-edits file-to-edit.js`.
  - flacoai will edit your file with the LLM's changes.
- Bugfix for creating new files.
- flacoai wrote 84% of the code in this release.  

### flacoai v0.61.0

- Load and save flacoai slash-commands to files:
...
- Anonymous, opt-in [analytics](https://flacoai.chat/docs/more/analytics.html) with no personal data sharing.
- flacoai follows litellm's `supports_vision` attribute to enable image support for models.
...
- Full support for Sonnet 10/22, the new SOTA model on flacoai's code editing benchmark.
  - flacoai uses Sonnet 10/22 by default.
...
- flacoai wrote 49% of the code in this release.
...
- Renamed `--yes` to `--yes-always`.
  - Now uses `flacoai_YES_ALWAYS` env var and `yes-always:` YAML key.
...
- [Use a pair of Architect/Editor models for improved coding](https://flacoai.chat/2024/09/26/architect.html)
...
- Numerous bug fixes.
- flacoai wrote 53% of the code in this release.

### flacoai v0.57.1

- Fixed dependency conflict between flacoai-chat[help] and [playwright].
...
- Support for OpenAI o1 models:
  ...
  - `flacoai --model o1-mini`
  - `flacoai --model o1-preview`
...
- flacoai wrote 70% of the code in this release.
...
- Numerous bug fixes for corner case crashes.
- flacoai wrote 56% of the code in this release.
...
- When users launch flacoai in their home dir, help them find/create a repo in a subdir.
...
- New `--upgrade` switch to install latest version of flacoai from pypi.
- Bugfix to `--show-prompt`.
- Disabled automatic reply to the LLM on `/undo` for all models.
- Removed pager from `/web` output.
- flacoai wrote 64% of the code in this release.

### flacoai v0.53.0

- [Keep your prompt cache from expiring](https://flacoai.chat/docs/usage/caching.html#preventing-cache-expiration) with `--cache-keepalive-pings`.
...
- New `--install-main-branch` to update to the latest dev version of flacoai.
...
- flacoai wrote 59% of the code in this release.
...
- flacoai now offers to run shell commands:
...
- Show the active chat mode at flacoai prompt.
...
- Bugfix: properly load `.flacoai.models.metadata.json` data.
...
- flacoai wrote 66% of the code in this release.
...
- When [scripting flacoai](https://flacoai.chat/docs/scripting.html) messages can now contain in-chat `/` commands.
- flacoai in docker image now suggests the correct command to update to latest version.
...
- Enhanced commit message attribution options:
  - New `--attribute-commit-message-author` to prefix commit messages with 'flacoai: ' if flacoai authored the changes, replaces `--attribute-commit-message`.
  - New `--attribute-commit-message-committer` to prefix all commit messages with 'flacoai: '.
- flacoai wrote 61% of the code in this release.
...
- Corrected instructions for upgrading the docker container to latest flacoai version.
- Removed obsolete 16k token limit on commit diffs, use per-model limits.

### flacoai v0.48.0

- Performance improvements for large/mono repos.
- Added `--subtree-only` to limit flacoai to current directory subtree.
...
- Support for Sonnet's 8k output window.
  - [flacoai already supported infinite output from Sonnet.](https://flacoai.chat/2024/07/01/sonnet-not-lazy.html)
- Workaround litellm bug for retrying API server errors.
- Upgraded dependencies, to pick up litellm bug fixes.
- flacoai wrote 44% of the code in this release.
...
- [Commit message](https://flacoai.chat/docs/git.html#commit-messages) improvements:
...
- [Linting](https://flacoai.chat/docs/usage/lint-test.html) improvements:
...
- Improved [docker support](https://flacoai.chat/docs/install/docker.html):
  - Resolved permission issues when using `docker run --user xxx`.
  - New `paulgauthier/flacoai-full` docker image, which includes all extras.
- Switching to code and ask mode no longer summarizes the chat history.
- Added graph of flacoai's contribution to each release.
...
- flacoai wrote 58% of the code in this release.
...
- Add `file: CONVENTIONS.md` to `.flacoai.conf.yml` to always load a specific file.
...
- Stop flacoai from asking to add the same url to the chat multiple times.
...
- flacoai wrote 45% of the code in this release.
...
- flacoai is better at offering to add files to the chat on Windows.
...
- flacoai wrote 42% of the code in this release.

### flacoai v0.44.0

- Default pip install size reduced by 3-12x.
- Added 3 package extras, which flacoai will offer to install when needed:
  - `flacoai-chat[help]`
  - `flacoai-chat[browser]`
  - `flacoai-chat[playwright]`
...
- flacoai wrote 29% of the code in this release (157/547 lines).
...
- Use `/help <question>` to [ask for help about using flacoai](https://flacoai.chat/docs/troubleshooting/support.html), customizing settings, troubleshooting, using LLMs, etc.
- Allow multiple use of `/undo`.
- All config/env/yml/json files now load from home, git root, cwd and named command line switch.
- New `$HOME/.flacoai/caches` dir for app-wide expendable caches.
- Default `--model-settings-file` is now `.flacoai.model.settings.yml`.
- Default `--model-metadata-file` is now `.flacoai.model.metadata.json`.
- Bugfix affecting launch with `--no-git`.
- flacoai wrote 9% of the 424 lines edited in this release.
...
- [Allow Claude 3.5 Sonnet to stream back >4k tokens!](https://flacoai.chat/2024/07/01/sonnet-not-lazy.html)
...
- flacoai now uses `claude-3-5-sonnet-20240620` by default if `ANTHROPIC_API_KEY` is set in the environment.
- [Enabled image support](https://flacoai.chat/docs/usage/images-urls.html) for 3.5 Sonnet and for GPT-4o & 3.5 Sonnet via OpenRouter (by @yamitzky).
- Added `--attribute-commit-message` to prefix flacoai's commit messages with "flacoai:".
...
- Options to suppress adding "(flacoai)" to the [git author and committer names](https://flacoai.chat/docs/git.html#commit-attribution).
...
- Use `--sonnet` for Claude 3.5 Sonnet, which is the top model on [flacoai's LLM code editing leaderboard](https://flacoai.chat/docs/leaderboards/#claude-35-sonnet-takes-the-top-spot).
- All `flacoai_xxx` environment variables can now be set in `.env` (by @jpshack-at-palomar).
- Use `--llm-history-file` to log raw messages sent to the LLM (by @daniel-vainsencher).
- Commit messages are no longer prefixed with "flacoai:". Instead the git author and committer names have "(flacoai)" added.

### flacoai v0.38.0

- Use `--vim` for [vim keybindings](https://flacoai.chat/docs/usage/commands.html#vi) in the chat.
- [Add LLM metadata](https://flacoai.chat/docs/llms/warnings.html#specifying-context-window-size-and-token-costs) via `.flacoai.models.json` file (by @caseymcc).
- More detailed [error messages on token limit errors](https://flacoai.chat/docs/troubleshooting/token-limits.html).
...
- Moved tests/ into flacoai/tests/.
...
- flacoai will notice if you paste a URL into the chat, and offer to scrape it.
- Performance improvements the repo map, especially in large repos.
- flacoai will not offer to add bare filenames like `make` or `run` which may just be words.
...
- [flacoai can now lint your code and fix any errors](https://flacoai.chat/2024/05/22/linting.html).
  - flacoai automatically lints and fixes after every LLM edit.
  - You can manually lint-and-fix files with `/lint` in the chat or `--lint` on the command line.
  - flacoai includes built in basic linters for all supported tree-sitter languages.
  - You can also configure flacoai to use your preferred linter with `--lint-cmd`.
- flacoai has additional support for running tests and fixing problems.
  - Configure your testing command with `--test-cmd`.
  - Run tests with `/test` or from the command line with `--test`.
  - flacoai will automatically attempt to fix any test failures.
  

### flacoai v0.35.0

- flacoai now uses GPT-4o by default.
  - GPT-4o tops the [flacoai LLM code editing leaderboard](https://flacoai.chat/docs/leaderboards/) at 72.9%, versus 68.4% for Opus.
  - GPT-4o takes second on [flacoai's refactoring leaderboard](https://flacoai.chat/docs/leaderboards/#code-refactoring-leaderboard) with 62.9%, versus Opus at 72.3%.
...
- Added native support for [Deepseek models](https://flacoai.chat/docs/llms.html#deepseek) using `DEEPSEEK_API_KEY` and `deepseek/deepseek-chat`, etc rather than as a generic OpenAI compatible API.

### flacoai v0.32.0

- [flacoai LLM code editing leaderboards](https://flacoai.chat/docs/leaderboards/) that rank popular models according to their ability to edit code.
...
- [flacoai is now also AI pair programming in your browser!](https://flacoai.chat/2024/05/02/browser.html) Use the `--browser` switch to launch an experimental browser based version of flacoai.
...
- Added [Gemini 1.5 Pro](https://flacoai.chat/docs/llms.html#free-models) as a recommended free model.
...
- Improved [model warnings](https://flacoai.chat/docs/llms.html#model-warnings) for unknown or unfamiliar models
...
- Added support for [directly connecting to Anthropic, Cohere, Gemini and many other LLM providers](https://flacoai.chat/docs/llms.html).
...
- New command line switches for working with popular models:
  ...
  - `--sonnet`
  - `--anthropic-api-key`
- Improved "whole" and "diff" backends to better support [Cohere's free to use Command-R+ model](https://flacoai.chat/docs/llms.html#cohere).
...
- Added support for new `gpt-4-turbo-2024-04-09` and `gpt-4-turbo` models.
  - Benchmarked at 61.7% on Exercism benchmark, comparable to `gpt-4-0613` and worse than the `gpt-4-preview-XXXX` models. See [recent Exercism benchmark results](https://flacoai.chat/2024/03/08/claude-3.html).
  - Benchmarked at 34.1% on the refactoring/laziness benchmark, significantly worse than the `gpt-4-preview-XXXX` models. See [recent refactor bencmark results](https://flacoai.chat/2024/01/25/benchmarks-0125.html).
  - flacoai continues to default to `gpt-4-1106-preview` as it performs best on both benchmarks, and significantly better on the refactoring/laziness benchmark.
...
- Issue a warning if user adds too much code to the chat.
  - https://flacoai.chat/docs/faq.html#how-can-i-add-all-the-files-to-the-chat
- Vocally refuse to add files to the chat that match `.flacoaiignore`
...
- Made past articles part of https://flacoai.chat/blog/
...
- [Significantly reduced "lazy" coding from GPT-4 Turbo due to new unified diff edit format](https://flacoai.chat/docs/unified-diffs.html)
  - Score improves from 20% to 61% on new "laziness benchmark".
  - flacoai now uses unified diffs by default for `gpt-4-1106-preview`.
...
- [Benchmarks for OpenAI's new 11/06 models](https://flacoai.chat/docs/benchmarks-1106.html)
- Streamlined [API for scripting flacoai, added docs](https://flacoai.chat/docs/faq.html#can-i-script-flacoai)
- Ask for more concise SEARCH/REPLACE blocks. [Benchmarked](https://flacoai.chat/docs/benchmarks.html) at 63.9%, no regression.
...
- [Improved repository map using tree-sitter](https://flacoai.chat/docs/repomap.html)
- Switched from "edit block" to "search/replace block", which reduced malformed edit blocks. [Benchmarked](https://flacoai.chat/docs/benchmarks.html) at 66.2%, no regression.
- Improved handling of malformed edit blocks targeting multiple edits to the same file. [Benchmarked](https://flacoai.chat/docs/benchmarks.html) at 65.4%, no regression.
- Bugfix to properly handle malformed `/add` wildcards.


### flacoai v0.15.0

- Added support for `.flacoaiignore` file, which instructs flacoai to ignore parts of the git repo.
- New `--commit` cmd line arg, which just commits all pending changes with a sensible commit message generated by gpt-3.5.
- Added universal ctags and multiple architectures to the [flacoai docker image](https://flacoai.chat/docs/install/docker.html)
- `/run` and `/git` now accept full shell commands, like: `/run (cd subdir; ls)`
- Restored missing `--encoding` cmd line switch.

### flacoai v0.14.2

- Easily [run flacoai from a docker image](https://flacoai.chat/docs/install/docker.html)
...
- Fixed /add bug in when flacoai launched in repo subdir
- Show models supported by api/key if requested model isn't available

### flacoai v0.14.0

- [Support for Claude2 and other LLMs via OpenRouter](https://flacoai.chat/docs/faq.html#accessing-other-llms-with-openrouter) by @joshuavial
- Documentation for [running the flacoai benchmarking suite](https://github.com/flacoai-AI/flacoai/tree/main/benchmark)
- flacoai now requires Python >= 3.9


### flacoai v0.13.0

- [Only git commit dirty files that GPT tries to edit](https://flacoai.chat/docs/faq.html#how-did-v0130-change-git-usage)
...
- Late-bind importing `sounddevice`, as it was slowing down flacoai startup
- Improved --foo/--no-foo switch handling for command line and yml config settings

### flacoai v0.12.0

- [Voice-to-code](https://flacoai.chat/docs/usage/voice.html) support, which allows you to code with your voice.
- Fixed bug where /diff was causing crash.
- Improved prompting for gpt-4, refactor of editblock coder.
- [Benchmarked](https://flacoai.chat/docs/benchmarks.html) at 63.2% for gpt-4/diff, no regression.
...
- [Benchmarked](https://flacoai.chat/docs/benchmarks.html) at 53.4% for gpt-3.5/whole (no regression).
...
- Stronger GPT-3.5 prompt against skipping/eliding code in replies (51.9% [benchmark](https://flacoai.chat/docs/benchmarks.html), no regression)
...
- Added `/git` command to run git from inside flacoai chats.
- Use Meta-ENTER (Esc+ENTER in some environments) to enter multiline chat messages.
- Create a `.gitignore` with `.flacoai*` to prevent users from accidentally adding flacoai files to git.
- Check pypi for newer versions and notify user.
- Updated keyboard interrupt logic so that 2 ^C in 2 seconds always forces flacoai to exit.
- Provide GPT with detailed error if it makes a bad edit block, ask for a retry.
- Force `--no-pretty` if flacoai detects it is running inside a VSCode terminal.
- [Benchmarked](https://flacoai.chat/docs/benchmarks.html) at 64.7% for gpt-4/diff (no regression)


### flacoai v0.9.0

- Support for the OpenAI models in [Azure](https://flacoai.chat/docs/faq.html#azure)
- Added `--show-repo-map`
...
- [Benchmarked](https://flacoai.chat/docs/benchmarks.html) at 53.8% for gpt-3.5-turbo/whole (no regression)

### flacoai v0.8.3

- Added `--dark-mode` and `--light-mode` to select colors optimized for terminal background
- Install docs link to [NeoVim plugin](https://github.com/joshuavial/flacoai.nvim) by @joshuavial
...
- Bugfix so that flacoai throws an exception when OpenAI returns InvalidRequest
...
- Added [FAQ](https://flacoai.chat/docs/faq.html)
...
- [Benchmark comparing code editing in GPT-3.5 and GPT-4](https://flacoai.chat/docs/benchmarks.html)
...
- Fixed a bug to allow flacoai to edit files that contain triple backtick fences.
