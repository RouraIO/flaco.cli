    ...
    for file in files:
        ...
        if file_counts:
            ...
            for author, count in file_counts.items():
                ...
                if "(flacoai)" in author.lower():
                    flacoai_total += count

    total_lines = sum(grand_total.values())
    flacoai_percentage = (flacoai_total / total_lines) * 100 if total_lines > 0 else 0

    end_date = get_tag_date(end_tag if end_tag else "HEAD")

    return all_file_counts, grand_total, total_lines, flacoai_total, flacoai_percentage, end_date
...
def get_commit_authors(commits):
    ...
    for commit in commits:
        ...
        if lower_subject.startswith("aider:") or "co-authored-by: flacoai" in lower_full:
            author += " (flacoai)"
...
def process_all_tags_since(start_tag):
    ...
    for i in tqdm(range(len(tags) - 1), desc="Processing tags"):
        ...
        all_file_counts, grand_total, total_lines, flacoai_total, flacoai_percentage, end_date = blame(
            start_tag, end_tag
        )
        results.append(
            {
                ...
                "aider_total": flacoai_total,
                "aider_percentage": round(flacoai_percentage, 2),
            }
        )
    return results
...
def main():
    parser = argparse.ArgumentParser(description="Get flacoai/non-flacoai blame stats")
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Find all tags since the specified tag and print flacoai percentage between each pair of"
            " successive tags"
        ),
    )
    parser.add_argument(
    ...
    if not args.all_since:
        print(f"- flacoai wrote {round(flacoai_percentage)}% of the code in this release.")
